export const presentationData = {
  ...
  slides: [
    ...
    },
    {
      ...
      bullets: [
        "Affine part \\(z = w^\\top x + b\\): each input \\(x_i\\) is weighted by \\(w_i\\); bias \\(b\\) shifts the decision boundary without changing orientation.",
        "Weights encode which patterns excite the neuron; training nudges \\(w, b\\) so that on labeled data the loss decreases.",
    ...
    },
    {
      ...
      bullets: [
        "Sigmoid \\(\\sigma(z)=1/(1+e^{-z})\\): outputs in \\((0,1)\\)—interpretable as probability for a single binary unit; derivative small when \\(|z|\\) is large (vanishing gradient).",
        "Tanh: outputs in \\((-1,1)\\), zero-centered—often nicer than sigmoid for hidden layers, still saturates.",
