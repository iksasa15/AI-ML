...
export const slides = [
  {
    ...
    "bullets": [
      {
        "text": "This section extracts the most important ideas into a compact teaching flow.",
        "icon": "check"
      },
      {
        "text": "Focus areas: transfer learning, model families, attention mechanics, scaling, and practical deployment.",
        "icon": "scaling"
      },
      {
        "text": "Designed for bootcamp delivery within roughly 20 slides.",
        "icon": "attention"
      }
    ],
  ...
  },
  {
    ...
    "bullets": [
      {
        "text": "BERT learns contextual token representations using both left and right context.",
        "icon": "token"
      },
      {
        "text": "Strong for understanding tasks: classification, NER, QA, and retrieval-oriented use cases.",
        "icon": "classification"
      },
      {
        "text": "Pretrained at scale (Wikipedia + BooksCorpus), then adapted to downstream tasks.",
        "icon": "train"
      }
    ],
  ...
  },
  {
    ...
    "bullets": [
      {
        "text": "[CLS] is prepended and its final embedding is used as a sequence-level summary.",
        "icon": "embedding"
      },
      {
        "text": "[SEP] separates sentence segments and marks boundaries.",
        "icon": "token"
      },
      {
        "text": "In classification tasks, [CLS] passes through a lightweight prediction head.",
        "icon": "classification"
      }
    ],
  ...
  },
  {
    ...
    "bullets": [
      {
        "text": "AraBERT follows BERT-base style architecture with Arabic-specific preprocessing.",
        "icon": "llm"
      },
      {
        "text": "Segmentation and Arabic-tailored tokenization improve vocabulary coverage.",
        "icon": "token"
      },
      {
        "text": "Domain/language adaptation quality can matter more than raw parameter count.",
        "icon": "llm"
      }
    ],
    "table": {
  ...
  },
  {
    ...
    "bullets": [
      {
        "text": "T5 reframes every NLP task as text input -> text output.",
        "icon": "idea"
      },
      {
        "text": "Task instruction is included as a prefix (e.g., \"summarize:\", \"translate:\").",
        "icon": "idea"
      },
      {
        "text": "One shared architecture/training recipe serves multiple tasks.",
        "icon": "train"
      }
    ],
  ...
  },
  {
    ...
    "bullets": [
      {
        "text": "Remove contiguous text spans from input and replace with sentinel tokens.",
        "icon": "token"
      },
      {
        "text": "Train model to reconstruct missing spans autoregressively.",
        "icon": "train"
      },
      {
        "text": "Encourages stronger generative behavior than token-only masking.",
        "icon": "token"
      }
    ],
    "table": {
  ...
  },
  {
    ...
    "bullets": [
      {
        "text": "GPT predicts next token from left context only (causal modeling).",
        "icon": "token"
      },
      {
        "text": "Uses masked self-attention in decoder stack for no-lookahead generation.",
        "icon": "attention"
      },
      {
        "text": "Excellent for open-ended completion, instruction following, and synthesis.",
        "icon": "llm"
      }
    ],
  ...
  },
  {
    ...
    "bullets": [
      {
        "text": "Attention solves fixed-vector bottlenecks in sequence transduction.",
        "icon": "embedding"
      },
      {
        "text": "At each step, model dynamically focuses on most relevant source positions.",
        "icon": "model"
      },
      {
        "text": "Entire mechanism is differentiable and learned end-to-end.",
        "icon": "attention"
      }
    ],
  ...
  },
  {
    ...
    "bullets": [
      {
        "text": "Compute relevance scores between query state and source states.",
        "icon": "attention"
      },
      {
        "text": "Apply softmax to obtain attention weights (probability distribution).",
        "icon": "attention"
      },
      {
        "text": "Return weighted sum of value/source states as contextual output.",
        "icon": "attention"
      }
    ],
  ...
  },
  {
    ...
    "bullets": [
      {
        "text": "Query asks for relevant context; Key indexes available context; Value carries content.",
        "icon": "check"
      },
      {
        "text": "Self-attention lets each token aggregate information from other tokens in sequence.",
        "icon": "token"
      },
      {
        "text": "Multi-head attention captures different linguistic relations in parallel.",
        "icon": "attention"
      }
    ],
  ...
  },
  {
    ...
    "bullets": [
      {
        "text": "Decoder cannot access future tokens during generation.",
        "icon": "token"
      },
      {
        "text": "Causal mask enforces left-to-right consistency and prevents information leakage.",
        "icon": "leakage"
      },
      {
        "text": "Enables parallel training over full target sequence while preserving autoregressive objective.",
        "icon": "train"
      }
    ],
  ...
  },
  {
    ...
    "bullets": [
      {
        "text": "Performance improves with data, model size, and compute, but with diminishing returns.",
        "icon": "model"
      },
      {
        "text": "Compute-optimal training balances parameter count and token budget.",
        "icon": "train"
      },
      {
        "text": "Chinchilla-style insight: many large models are under-trained relative to their size.",
        "icon": "train"
      }
    ],
    "table": {
  ...
  },
  {
    ...
    "bullets": [
      {
        "text": "Choose architecture by task type: encoder-only, encoder-decoder, or decoder-only.",
        "icon": "encoding"
      },
      {
        "text": "Prefer transfer learning and fine-tuning over training from scratch when possible.",
        "icon": "train"
      },
      {
        "text": "Use attention-aware designs and decoding strategy (greedy/beam) based on product constraints.",
        "icon": "train"
      },
      {
        "text": "Plan for memory/compute early: quantization, context limits, and deployment targets.",
        "icon": "monitoring"
      },
      {
        "text": "For domain-heavy use cases, adapt with specialized corpora and evaluation protocols.",
        "icon": "test"
      }
    ],
