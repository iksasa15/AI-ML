...
def build_registry() -> None:
    ...
    register(
        ...
        subtitle="مخرج واضح لكل هدف — مو شعارات عامة",
        items=[
            "تصوغ أمرًا كامل العناصر يقلّل الهلوسة (سياق + دور + جمهور + شكل + قيود)",
            "تحوّل نقاطًا خامًا إلى إيميل وتقرير ومنشور جاهز للإرسال",
            "تنتج صورة/عرض/فيديو بأمر منضبط دون خبرة تصميم مسبقة",
            "تحلّل جدولًا بلغة بشرية وتبني مسار أتمتة بسيط واحد",
            "تبني مساعدًا شخصيًا مع قاعدة أمان: لا ترفع أسرارًا غير معتمدة",
        ],
    )
    register(
        "day_divider",
        day="اليوم الأول",
        title="هندسة الأوامر · المحتوى اليومي · البحث الموثّق",
        photo="hero-day1.jpg",
        sessions=[
            ("الجلسة 1", "فهم النموذج وهندسة الأوامر", "ساعة ونصف"),
            ("الجلسة 2", "محتوى العمل اليومي", "ساعة"),
            ("الجلسة 3", "بحث وملفات + تطبيق ختامي", "ساعة ونصف"),
        ],
    )
    ...
    register(
        ...
        title="فهم النموذج وهندسة الأوامر",
        duration="ساعة ونصف",
        goal="بنهاية الجلسة: قالب أمر شخصي تنسخه غدًا لأي مهمة عمل",
        photo="hero-laptop.jpg",
    )
    register(
        "cards",
        kicker="اليوم 1  ·  الجلسة 1",
        title="ثلاث خرافات شائعة",
        subtitle="صحّح التوقّع قبل ما تبدأ تكتب أوامر",
        cards=[
            ("«يفكر مثلي»", "لا. يكمل نصًا بالاحتمال الأعلى — مو فهم بشري."),
            ("«دايم صحيح»", "لا. كثير ممتاز… والحساس يحتاج تحقق."),
            ("«يغني عن المراجعة»", "لا. التوقيع على الإرسال مسؤوليتك أنت."),
        ],
    )
    register(
        "cards",
        kicker="اليوم 1  ·  الجلسة 1",
        title="تقليدي مقابل توليدي",
        subtitle="متى تستخدم أي نوع في شغلك؟",
        cards=[
            ("تقليدي", "تصنيف، كشف احتيال، توصية من كتالوج — مخرج ضيق: فئة أو رقم."),
            ("توليدي", "من نقاط → إيميل، من تقرير → ملخص، من فكرة → سكربت."),
            ("الخلاصة", "التوليدي مرن… وغير مضمون. الأمر + المراجعة جزء من العمل."),
        ],
    )
    register(
        "cards",
        kicker="اليوم 1  ·  الجلسة 1",
        title="كيف يعمل النموذج؟ وأين الهلوسة؟",
        subtitle="فكرة واحدة تثبت طول الورشة",
        cards=[
            ("التنبؤ", "يكمل الكلمة التالية حسب التدريب + أمرك."),
            ("الهلوسة", "كلام يبدو صحيحًا وهو مختلق — خاصة الأرقام والأسماء."),
            ("العلاج المبكر", "قيد «لا أعرف»، اعتمد على ملف/مصدر، ثم راجع بشريًا."),
        ],
    )
    register(
        "cards",
        kicker="اليوم 1  ·  الجلسة 1",
        title="الأمر = موجّه عمل",
        subtitle="جودة الناتج تتبع جودة التوجيه — لا اسم الأداة",
        cards=[
            ("بدون موجّه", "موظف جديد يكتب كلامًا عامًا… والنموذج كذلك."),
            ("مع موجّه", "من أنت؟ لمن؟ بأي شكل؟ وما الممنوع؟"),
            ("القاعدة", "افحص الأمر قبل ما تلوم الأداة."),
        ],
    )
    register(
        "diagram",
        kicker="اليوم 1  ·  الجلسة 1",
        title="العناصر الخمسة للأمر",
        subtitle="نظرة واحدة — ثم نفصّل بأمثلة",
        image="prompt-structure.png",
    )
    register(
        "bullets",
        kicker="اليوم 1  ·  الجلسة 1",
        title="تعميق: السياق والدور",
        subtitle="مثال فرع الرياض — سيء مقابل أفضل",
        items=[
            "سياق سيء: «لخّص التقرير» — النموذج لا يعرف أي ملف ولا ماذا يهم",
            "سياق أفضل: مبيعات أغسطس · فرع الرياض · هدف 1.2م · متحقق 980 ألف · ركّز على الفجوة",
            "دور سيء: بدون دور → نبرة عشوائية",
            "دور أفضل: «أنت محلل إداري» غير «أنت كاتب إعلانات» على نفس المدخل",
            "اختبر: غيّر الدور فقط ولاحظ اختلاف المفردات وعمق التحليل",
        ],
    )
    register(
        "bullets",
        kicker="اليوم 1  ·  الجلسة 1",
        title="تعميق: الجمهور · الشكل · القيود",
        subtitle="ثلاث قرارات تحوّل الكلام إلى مخرج يُستخدم",
        items=[
            "جمهور: مدير = مختصر + قرار · فريق = تفاصيل · منشور عام = بلا أرقام سرية",
            "شكل: 5 نقاط · جدول مشكلة/إجراء · إيميل جاهز للنسخ — لا تترك الشكل مفتوحًا",
            "قيود ذهبية: لا تخترع أرقامًا · قل لا أعرف إن نقص · حدّد الطول واللهجة",
    ...
    register(
        "diagram",
        kicker="اليوم 1  ·  الجلسة 1",
        title="قبل وبعد: أمر ضعيف مقابل قوي",
        subtitle="نفس الأداة · أمران · جودتان — الإثبات قبل القالب",
    ...
    register(
        ...
        subtitle="انسخ الحقول الستة الآن — حوّل الجودة من حظ إلى عادة",
        items=[
            ...
            "القيود: الطول · اللهجة · لا تخترع · قل لا أعرف إن لم تكن متأكدًا",
    ...
    register(
        "diagram",
        kicker="اليوم 1  ·  الجلسة 1",
        title="مثال ممتلئ — فرع الرياض",
        subtitle="ابدأ من قالب جاهز وعدّل القيم فقط",
    ...
    register(
        ...
        subtitle="تفكير خطوة بخطوة — متى ولماذا",
        cards=[
            ("استخدمه", "مقارنة خيارين · توصية من أرقام · تلخيص ثم استنتاج."),
            ("الصيغة", "اكتب خطواتك في 3–5 أسطر ثم أعطِ النتيجة فقط."),
            ("لا تستخدمه", "إيميل قصير أو إعادة صياغة بسيطة — يطوّل بلا فائدة."),
        ],
    )
    register(
        "cards",
        ...
        subtitle="أمثلة داخل الأمر لتثبيت النبرة والشكل",
        cards=[
            ("استخدمه", "إيميلات دعم بنفس الأسلوب · محاضر · منشور أسبوعي ثابت."),
            ("الهيكل", "مثال 1 مدخل/مخرج · مثال 2 · ثم: نفّذ على هذا بنفس الأسلوب."),
            ("تجنّب", "لصق نص طويل بلا عناوين أمثلة — سمّها بوضوح."),
        ],
    )
    register(
        "cards",
        kicker="اليوم 1  ·  الجلسة 1",
        title="درع ضد الهلوسة",
        subtitle="ثلاث طبقات — مرة واحدة وتُكرَّر طول الورشة",
        cards=[
            ("1) داخل الأمر", "قيود دقة + «قل لا أعرف إن لم تكن متأكدًا»."),
            ("2) في الأداة", "مصدر بروابط أو الاعتماد على الملف المرفوع فقط."),
    ...
    register(
        "tools",
        kicker="اليوم 1  ·  الجلسة 1",
        title="اختر الأداة حسب المهمة",
        subtitle="لا تعصّب لأداة — المعيار: المهمة + الملف + هل تحتاج ويب؟",
        tools=[
            ("ChatGPT", "صياغة يومية · تجارب سريعة · مساعد عام"),
            ("Claude", "ملفات طويلة · أسلوب منضبط · تقارير كثيفة"),
            ("Google Gemini", "بحث/وسائط متعددة · سياق ويب في مسار واحد"),
        ],
    )
    register(
        "bullets",
        kicker="اليوم 1  ·  الجلسة 1",
        title="عيادة أخطاء شائعة",
        subtitle="كل خطأ + علاج مختصر",
        items=[
            "غامض بلا هدف → اكتب: أريد قرارًا / إيميلًا / ملخصًا عن…",
            "بلا جمهور → حدّد القارئ ومستوى الرسمية صراحة",
            "بلا شكل → اطلب نقاطًا أو جدولًا أو هيكل إيميل",
            "قبول بلا تحقق → دقيقة واحدة للأرقام قبل الإرسال",
            "طويل بلا أولوية → المهمة أولًا ثم السياق ثم الزوائد",
        ],
    )
    register(
        "diagram",
        kicker="اليوم 1  ·  الجلسة 1",
        title="مسار العمل الثابت",
        subtitle="عادة تتكرر في كل تمرين",
    ...
    register(
        ...
        title="محتوى العمل اليومي",
        duration="ساعة",
        goal="من نقاط خام إلى إيميل وتقرير ومنشور جاهز للإرسال",
        photo="hero-day1.jpg",
    )
    register(
        "bullets",
        kicker="اليوم 1  ·  الجلسة 2",
        title="إيميل يحفّز إجراءً",
        subtitle="هيكل ثابت يمنع إيميل «للعلم» بلا حركة",
        items=[
            "الموضوع + التحية + الغرض في سطر واحد",
            "ثلاث نقاط كحد أقصى — واضحة وقابلة للمسح البصري",
            "طلب إجراء واحد فقط: قرار · موعد · اعتماد · رد",
            "خاتمة قصيرة · اللهجة حسب الجمهور (مؤسسية أو ودّية مهنية)",
            "قيد: لا تخترع أرقامًا أو مواعيد غير موجودة في المدخل",
        ],
    )
    register(
        "bullets",
        kicker="اليوم 1  ·  الجلسة 2",
        title="تقرير يقرأه المدير",
        subtitle="الملخص التنفيذي أولًا — لا تبدأ من الصفحة 3",
        items=[
            ...
            "تجنّب السرد الطويل بلا قرار مطلوب",
        ],
    )
    register(
        "cards",
        kicker="اليوم 1  ·  الجلسة 2",
        title="إعادة صياغة مع قفل المعنى",
        subtitle="حسّن الوضوح… بدون تجميل يغيّر الحقائق",
        cards=[
            ("القيد الذهبي", "حافظ على المعنى والأرقام؛ حسّن الوضوح فقط."),
            ("الخطر", "بدون القيد: يزيد رقمًا أو يلطّف مشكلة «للتجميل»."),
            ("الاستخدام", "مسودة ضعيفة → نص مهني جاهز للمشاركة الداخلية."),
        ],
    )
    register(
        "diagram",
        kicker="اليوم 1  ·  الجلسة 2",
        title="فكرة واحدة → ثلاثة جماهير",
        subtitle="نفس الحقيقة · رسائل مختلفة · بلا نسخ حرفي بين المنصات",
        image="one-idea-three-audiences.png",
    )
    register(
        "cards",
        kicker="اليوم 1  ·  الجلسة 2",
        title="قوالب يومية — انسخ واملأ",
        subtitle="تمرين مقترح: مسودة اجتماع خام → الإيميل + التقرير + المنشور",
        cards=[
            ("إيميل", "موضوع · تحية · غرض · 3 نقاط · طلب إجراء · خاتمة"),
            ("تقرير", "ملخص تنفيذي · ما حدث · أرقام · مخاطر · توصية"),
            ("منشور", "خطاف · فائدة واحدة · دليل قصير · سؤال — بلا أرقام سرية"),
        ],
    )

    # ─── 28–31 جلسة 3 + تطبيق + ختام ───
    register(
        "session_open",
    ...
    register(
        "tools",
        kicker="اليوم 1  ·  الجلسة 3",
        title="أدوات ومسار عملي",
        subtitle="ابحث/لخّص بمصدر → انسخ الموثوق → حوّل لإيميل أو منشور",
        tools=[
            ("Perplexity AI", "بحث حي بروابط قابلة للفتح والمراجعة"),
            ("ChatPDF / Claude", "رفع PDF طويل واستخراج أقسام محددة"),
    ...
    register(
        "diagram",
        kicker="اليوم 1  ·  الجلسة 3",
        title="بوابة جودة البحث",
        subtitle="قبل الإرسال: نعم/لا على كل صندوق — وإلا لا ترسل",
        image="research-quality-gate.png",
    )
    register(
        "steps",
        kicker="اليوم 1  ·  ختام وتطبيق",
        title="تمرين المساعد الإداري + ختام اليوم",
        subtitle="ملف واحد → ملخص · إيميل · 3 منشورات · ثم راجع",
        steps=[
            "لخّص الملف في 5 نقاط + توصية (لا تخترع أرقامًا)",
            "حوّل النقاط إلى إيميل رسمي بطلب إجراء واحد",
            "اكتب 3 منشورات عامة بلا بيانات سرية ثم طبّق بوابة الجودة",
