...
def add_rtl_text(
    ...
    *,
    ...
    try:
        ...
        pass
    chunks = [part for part in (value or "").split("\n")]
    if not chunks:
        chunks = [""]
...
def rtl_bullets(slide, items: list[str], *, top=Inches(2.2), size=17, pitch=0.58, left=None, width=None):
    ...
    for i, item in enumerate(items):
        ...
        # bullet on the right for RTL feel
        add_rtl_text(
            slide,
...
def paint_steps(prs, spec: dict, page: int, total: int):
    ...
    for i, text in enumerate(steps):
        ...
        soft_card(s, MARGIN, y, Inches(12.1), Inches(0.9), fill=SOFT if i % 2 == 0 else SOFT_2)
        # number circle via rounded small card
        soft_card(s, Inches(11.35), y + Inches(0.18), Inches(0.55), Inches(0.55), fill=PRIMARY)
        add_text(
            s, Inches(11.35), y + Inches(0.22), Inches(0.55), Inches(0.5),
            str(i + 1), size=16, bold=True, color=WHITE, align=PP_ALIGN.CENTER,
        )
        add_rtl_text(
            s, MARGIN + Inches(0.35), y + Inches(0.22), Inches(10.7), Inches(0.55),
            text, size=16, color=INK, anchor=MSO_ANCHOR.MIDDLE,
        )
